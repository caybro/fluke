/*
    Copyright (C) 2014-2018 Lukáš Tinkl <lukas@kde.org>
...
void GeoLocation::init()
{
    ...
    if (clientPath.isValid()) {
        ...
        m_clientIface->setProperty("RequestedAccuracyLevel", 16); // exact
...
void GeoLocation::onLocationUpdated(const QDBusObjectPath &oldPath, const QDBusObjectPath &newPath)
{
    Q_UNUSED(oldPath)
    const QString path = newPath.path();
    //qDebug() << "Got new location object path" << newPath.path();
